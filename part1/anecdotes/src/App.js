...
const App = () => {
  ...
  const [voteObj, setVote] = useState([0,0,0,0,0,0,0])
  const increaseVote = () =>{setVote(prevState => {
const copy = [...voteObj]
 copy[selected]= copy[selected]+1 
return copy})}
  const nextAnecdote = () =>{return setSelected(Math.floor(Math.random()*anecdotes.length))}
  return (
    <div>
      
      <h1>Anecdote of the day</h1>
      {anecdotes[selected]}
      <p>has {voteObj[selected]} votes</p>
      ...
      <p>{anecdotes[voteObj.indexOf(Math.max(...voteObj))]}</p>
     
      <p>has {voteObj[voteObj.indexOf(Math.max(...voteObj))]} votes</p>
    </div>
  )
}
